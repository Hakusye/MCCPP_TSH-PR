...
class MCCPP {
  protected:
    ...
    static long long DiffEvalFunction(const ColorSet &color_set,const int &change_vertex,const int &after_color);
    //新しい方が計算時間がすくない(O(nlogn) -> O(n)),バグが見つかり次第戻す
    static std::set<int> AdjacentColorSet(const std::set<int> &vertexes_set,const int &vertex);
    static std::set<int> OldAdjacentColorSet(const std::set<int> &vertexes_set,const int &vertex);
  ...
  public:
    ...
    static long long EvalFunction(const ColorSet &color_set); //あとでprotected
    MCCPP();
    ~MCCPP();
    static void Run();
    ...
    static void InputGraph(const std::string &filename);
    static void InitGraph(std::ifstream& inputfile);
    static void InputColorSet();
    static void InputColorSet(const std::string &filename);
    static void InitColorSet(std::ifstream& inputfile);
    static void _ShowGraph();
    static void _ShowColorSet(const ColorSet &color_set);
};